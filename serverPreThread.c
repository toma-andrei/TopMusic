...
pthread_mutex_t lockLogin = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t lockVote = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t locktopGeneral = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t lockAdminFile = PTHREAD_MUTEX_INITIALIZER;
...
void raspunde(int client, int idThread)
{
    ...
    while (connected)
    {
        ...
        if (!loggedIn) // daca clientul nu e logat
        {
            ...
            {
                loggedIn = loginApproval(client, &lockLogin, msgFromClient, length);
                ...
                if (loggedIn)
                {
                    ...
                    printf("%s\n", username);
                    fflush(stdout);
                    adminRight = isAdmin(client, &lockLogin, msgFromClient, length);
                }
                continue;
            }
        ...
        else if (loggedIn) // daca clientul este logat
        {
            ...
            {
                voteSong(client, username, msgFromClient, length, &lockVote);
                continue;
            }

            if (strstr(msgFromClient, "top general"))
            {
                topGeneral(client, &locktopGeneral, msgFromClient, length);
                continue;
            }

            if (strstr(msgFromClient, "top for "))
            {
                topForGenre(client, &locktopGeneral, msgFromClient, length);
                continue;
            }

            if (strstr(msgFromClient, "add comment ("))
            {
                addComment(client, username, msgFromClient, length, &lockVote);
                continue;
            }

            if (strstr(msgFromClient, "comments for "))
            {
                showComment(client, &lockVote, msgFromClient, length);
                continue;
            }
            ...
            if (strstr(msgFromClient, "deny voting for "))
            {
                ...
                restrictVote(client, &lockAdminFile, msgFromClient, length);
                continue;
            }
            ...
            if (strstr(msgFromClient, "delete song "))
            {
                ...
                deleteSong(client, &lockAdminFile, msgFromClient, length);
                continue;
            }
            ...
            if (strstr(msgFromClient, "accept admin "))
            {
                ...
                acceptAdminRequest(client, &lockAdminFile, msgFromClient, length);
                continue;
            }
            ...
            if (strstr(msgFromClient, "get admin req list"))
            {
                ...
                getAdmReqList(client, &lockAdminFile, msgFromClient, length);
                continue;
            }

            if (strstr(msgFromClient, "delete comment "))
            {
                if (!adminRight)
                {
                    noAdmRight(client);
                ...
                deleteComment(client, idThread, msgFromClient, length);
                continue;
            }
            ...
            if (strstr(msgFromClient, "reject admin "))
            {
                ...
                rejectAdminRequest(client, &lockAdminFile, msgFromClient, length);
                continue;
            }
